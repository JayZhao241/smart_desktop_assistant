...
void lcd_init(void)
{
    /* 1. 硬件底层（时钟、GPIO、FSMC时序）已经在 MX_FSMC_Init() 中完成 */
    /* 我们只需要初始化背光引脚（如果 MX_GPIO_Init 中未包含） */
        ...
        if (lcddev.id != 0x7789)        /* 也不是ST7789, 尝试是不是 NT35310 */
        {
            ...
            if (lcddev.id != 0x5310)    /* 也不是NT35310,尝试看看是不是ST7796 */
            {
                ...
                if (lcddev.id != 0x7796)    /* 也不是ST7796,尝试看看是不是NT35510 */
                {
                    ...
                    HAL_Delay(5);                /* 等待5ms, 让1963复位完成再操作 */
    ...
    /* 打印 LCD ID（需自行实现 printf 重定向） */
    printf("LCD ID:%x\r\n", lcddev.id); 

    /* 3. 根据检测到的 ID 执行特定寄存器初始化 */
    ...
    /* 这里的 hsram1 需与 fsmc.h/c 中的句柄名称一致 */
